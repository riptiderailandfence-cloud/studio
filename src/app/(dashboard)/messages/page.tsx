...
export default function MessagesPage() {
  ...
  const simulateIncomingMessage = () => {
    if (!tenantId || !activeThreadId) return;
    const messageColRef = collection(firestore, 'tenants', tenantId, 'chats', activeThreadId, 'messages');
    addDocumentNonBlocking(messageColRef, {
      senderId: 'client',
      text: "Thanks for the estimate! When can your crew start?",
      timestamp: serverTimestamp(),
      status: 'delivered'
    });
    ...
    const chatDocRef = doc(firestore, 'tenants', tenantId, 'chats', activeThreadId);
    updateDocumentNonBlocking(chatDocRef, {
      lastMessage: "Thanks for the estimate! When can your crew start?",
      lastTimestamp: serverTimestamp(),
      updatedAt: serverTimestamp(),
      unreadCount: (activeThread?.unreadCount || 0) + 1
    });
    
    toast({ title: "Mock SMS Received", description: "Simulation successful." });
  };
      ...
      <div className="w-80 flex flex-col border-r bg-slate-50/50">
        <div className="p-4 border-b bg-background space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="gap-1.5 font-bold text-[10px] uppercase">
            ...
            {filteredThreads.map((thread) => (
              ...
              >
                ...
                {thread.unreadCount > 0 && (
                  <Badge className="absolute top-4 right-4 h-5 w-5 flex items-center justify-center p-0 rounded-full animate-bounce">
                    {thread.unreadCount}
                  </Badge>
                )}
              </button>
      ...
      <div className="flex-1 flex flex-col bg-background">
        <div className="p-4 border-b flex items-center justify-between bg-background">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                  <Smartphone className="h-2.5 w-2.5" /> {activeThread.customerPhone || 'Direct Message'}
                </p>
              </div>
            </div>
          ) : (
          ...
          <div className="flex items-center gap-2">
            {activeThread && (
              <Button variant="outline" size="sm" className="gap-2 text-[10px] font-bold h-8 uppercase" onClick={simulateIncomingMessage}>
                <Zap className="h-3 w-3 text-amber-500 fill-amber-500" /> Simulate SMS
              </Button>
            )}
